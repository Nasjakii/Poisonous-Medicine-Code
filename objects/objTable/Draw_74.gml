...
for(var i = 0; i < inventory_size / 2; i++) {
	...
	if inventory_local[i] != -1 {
		//Draw sprite at right size
		var spr_item = inventory_local[i].sprite;
	...
	if inventory_local[i+inventory_size / 2] != -1 {
		//Draw sprite at right size
	 	var spr_item = inventory_local[i].sprite;
		var size = scr_relative_size(sprite_get_width(spr_item), sprite_get_height(spr_item), itembox_size);
...
#region Pick up
	...
	if mouse_check_button_pressed(mb_left) {
		...
		for(i = 0; i < inventory_size; i++) {
			if itembox_h[i] && inventory_local[i] != -1{
				//hold the item array
				global.item_holding = inventory_local[i];
				//delete item from local array
				inventory_array_local[i] = -1;
			...
			for(var i = 0; i < array_length(desk_array); i++) {
				if desk_array[i,0] != -1 {
					...
					if scr_in_bounds(mouse_x,mouse_y,xpos,ypos,xpos2,ypos2) {
						...
						global.item_holding = desk_array[i,0];
...
if mouse_check_button_released(mb_left) {
	...
	//over desk ?
	if global.item_holding != -1 && scr_in_bounds(mouse_x_gui, mouse_y_gui, desk_gui_x, desk_gui_y, desk_gui_x + desk_gui_w, desk_gui_y + desk_gui_h) {
		for(var i = 0; i < array_length(desk_array); i++) {
			//free slot
			if desk_array[i,0] == -1 {
				
				//Create instance with all atributes 
				desk_array[i,3] = scr_create_crafting_instance(global.item_holding.sprite, mouse_x, mouse_y, scale, global.item_holding);	
...
if global.item_holding != -1 {
	var spr = global.item_holding.sprite;
